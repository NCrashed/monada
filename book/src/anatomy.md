...
| `host_api` | The script-API version the map requires — the version of the function set your scripts call (`entity_create`, `voxel_set`, …). A host runs only a version inside its supported range (`11..=11` today) and refuses the map up front instead of failing mid-game. Growth is additive, so the range's top moves with each new verb; a *breaking* change moves the bottom too, which is what makes an old map's declaration a loud refusal rather than a quiet re-interpretation. Omitted = `1` — below today's floor, so declare the version. |
