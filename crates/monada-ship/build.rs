//! Pack `map/` into a deterministic `ship.monada` archive in `OUT_DIR`,
...
//! (Identical to monada-rpg's build.rs but for `ship.monada`.)
...
fn main() {
    ...
    let bytes = monada_format::pack(&files).expect("pack ship.monada");
    let out_dir = std::env::var("OUT_DIR").expect("OUT_DIR");
    fs::write(Path::new(&out_dir).join("ship.monada"), bytes).expect("write ship.monada");
}
