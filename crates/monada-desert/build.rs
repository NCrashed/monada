//! Pack `map/` into a deterministic `desert.monada` archive in `OUT_DIR`,
...
//! (Identical to monada-rpg's build.rs but for `desert.monada`.)
...
fn main() {
    ...
    // Watch the tree itself so newly-added assets re-trigger the pack, not
    // just edits to files that already existed.
    println!("cargo:rerun-if-changed={}", map_dir.display());

    let mut files = BTreeMap::new();
    collect(&map_dir, &map_dir, &mut files);

    let bytes = monada_format::pack(&files).expect("pack desert.monada");
    let out_dir = std::env::var("OUT_DIR").expect("OUT_DIR");
    fs::write(Path::new(&out_dir).join("desert.monada"), bytes).expect("write desert.monada");
}
