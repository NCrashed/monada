//! Pack `map/` into a deterministic `rts.monada` archive in `OUT_DIR`,
...
//! (Identical to monada-rpg's build.rs but for `rts.monada`.)
...
fn main() {
    ...
    let bytes = monada_format::pack(&files).expect("pack rts.monada");
    let out_dir = std::env::var("OUT_DIR").expect("OUT_DIR");
    fs::write(Path::new(&out_dir).join("rts.monada"), bytes).expect("write rts.monada");
}
